...
public class Benchmark {
    ...
    public void generateInput(Generation_Mode mode, int ratio) {
        ...
        this.ratio = (mode == Generation_Mode.Nearly) ? ratio : null;
        switch (mode) {
            case Fully_Random:
    ...
    public void setInputData(Generation_Mode mode, int ratio, ArrayList<Integer> input, HashMap<Integer, Integer> inputMap) {
        this.generationMode = mode;
        this.ratio = (mode == Generation_Mode.Nearly) ? ratio : null;
    ...
    public void delete() {
        ...
        ArrayList<Integer> insertedValues = new ArrayList<>(inputMap.keySet());
        int numOfDeletions = (int) (0.20 * insertedValues.size());
        Collections.shuffle(insertedValues, random);

        long startTime = System.nanoTime();
        for (int i = 0; i < numOfDeletions; i++) {
            tree.delete(insertedValues.get(i));
        }
        long endTime = System.nanoTime();
    ...
    public Benchmark_Record run() {
        ...
        this.contains();
        this.delete();
        return new Benchmark_Record(treeType, generationMode, ratio, insertionTime, treeSortTime,
                baselineSortTime, containsExistingTime, containsNonExistingTime, deletionTime, traversalTime, treeHeight);
    }
